...
contract Lottery is ILottery, Ticket, LotterySetup, ReferralSystem, RNSourceController {
        ...
        RNSourceController(maxRNFailedAttempts, maxRNRequestDelay)
    {
        ...
        // @note - initial supply goes all to deployer
        // @note - deployer then splits up in correct amounts, sends to LBP, CEX, DEX, stakes and locks for devs, etc.
    ...
    function claimable(uint256 ticketId) external view override returns (uint256 claimableAmount, uint8 winTier) {
        ...
        if (!ticketInfo.claimed) {
            ...
            // @audit-issue Winning tickets have a one-year deadline from the draw time to claim their prize by
            // redeeming the winning NFT ticket.
            // Here they have one year time from the cooldown period, not deadline
    ...
    function receiveRandomNumber(uint256 randomNumber) internal override onlyWhenExecutingDraw {
        // @note - compute winning ticket, start a new draw cycle and see how many people won jackpot
        ...
        // @note - either jackpot is won, or other winners are paid. Set winAmounts accordingly
        if (jackpotWinners > 0) {
        ...
        // @note - recompute new currentNetProfit, store winning ticket, reset drawExecutionInProgress
    ...
    function claimWinningTicket(uint256 ticketId) private onlyTicketOwner(ticketId) returns (uint256 claimedAmount) {
        ...
        // @audit-issue - redundant, as it seems it is only used to figure out between how many players to split pot?
    ...
    // @audit-issue Why do we only return jackpot to pot? What happens with the other unclaimed tickets, and their
    // winnings
    // Are they stuck in the smart contract now?
